...
(hugsql/set-adapter! (next-adapter/hugsql-adapter-next-jdbc {:builder-fn rs/as-kebab-maps}))
        ...
        (fact "post-query"
              ...
              (fact "remove-ks :id"
                    (->> "select id from test order by id;"
                         (db/query db ^:opts {[:remove-ks :post] #{:test/id}})
                         (map :test/id))
                    => [nil nil])
              (fact "remove :test/id"
                    (->> "select id from test order by id;"
              ...
              (fact "insert with scalar return value"
                    (let [r (db/query! db "insert into test values (-1);")
                          ids (->> sql-fn-query
                                   (db/query db)
                                   (map :test/id))]
                      [r ids]) => [[{:next.jdbc/update-count 1}] [-1 0 42]])
              (fact "delete"
                    (let [r (db/query! db "delete from test where id = -1;")
                          ids (->> sql-fn-query
                                   (db/query db)
                                   (map :test/id))]
                      [r ids] => [[{:next.jdbc/update-count 1}] [0 42]])))
              ...
              (fact "insert with return scalar value"
                    (let [r (db/query! db ["insert into test values (?);" -4])
                          ids (->> sql-fn-query
                                   (db/query db)
                                   (map :test/id))]
                      [r ids]) => [[{:next.jdbc/update-count 1}] [-4 0 42]])
              (fact "delete"
              ...
              (fact "insert with scalar return value"
                    (let [r (db/query! db {:insert-into :test
                          ...
                          ids (->> sql-fn-query
                                   (db/query db)
                                   (map :test/id))]
                      [r ids]) => [[{:next.jdbc/update-count 1}] [-3 0 42]])
              (fact "insert with return value"
              ...
              (fact "delete"
                    (let [r (db/query! db {:delete-from :test
                          ...
                          ids (->> sql-fn-query
                                   (db/query db)
                                   (map :test/id))]
                      [r ids] => [[{:next.jdbc/update-count 1}] [-2 0 42]])))
        ...
        (fact "batch inserts"
              ...
              (try
                (db/query! db {:insert-into :test :values [{:id 3}
                                                           ...
                                                           {:id "5" :foobar "Asdf"}]})
                (catch Exception e
                  (not (zero? (count (:messages (ex-data e))))))) => true)
        (fact "error-maps"
              (fact "no args"
                    (try
